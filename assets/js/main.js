...
(function() {
  ...
  if (mobileNavToggleBtn) {
    ...
    mobileNavToggleBtn.setAttribute('aria-label', 'Buka menu');
  ...
  document.addEventListener('click', (event) => {
    ...
    if (navmenu.contains(event.target) || event.target === mobileNavToggleBtn) return;

    mobileNavToogle();
  });

  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && document.querySelector('body').classList.contains('mobile-nav-active')) {
      mobileNavToogle();
    }
  });
